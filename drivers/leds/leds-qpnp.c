...
#if defined(CONFIG_MACH_MSM8974_B1_KR) || defined(CONFIG_MACH_MSM8974_B1W)
	if (!init_patterns_for_window_color) {
...
void make_blink_led_pattern(int rgb, int delay_on, int delay_off)
{
	...
	if (rgb_devider) {
		...
		switch (window_color)
		{
			...
			case WINDOW_COLOR_TK:
			default:
				blink_pattern[0] = (((rgb >> 16) & 0xFF)/17)/rgb_devider;
				blink_pattern[2] = (((rgb >> 8) & 0xFF)/17)/rgb_devider;
				break;
		}
#else
		...
		switch (window_color)
		{
			...
			case WINDOW_COLOR_TK:
			default:
				blink_pattern[4] = ((rgb & 0xFF)/17)/rgb_devider;
				break;
		}
#else
...
void make_onoff_led_pattern(int rgb)
{
	...
	*/
	...
	switch (window_color)
	{
		...
		case WINDOW_COLOR_TK:
		default:
			onoff_pattern[0] = ((rgb >> 16) & 0xFF)/17;
			onoff_pattern[2] = ((rgb >> 8) & 0xFF)/17;
			break;
	}
#else
