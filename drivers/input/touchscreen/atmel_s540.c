...
#ifdef MXT_LPWG
#define MXT_LATEST_CONFIG_CRC	0xCDAEEF
u8 latest_firmware[] = {
...
static void mxt_proc_t100_anti_message(struct mxt_data *data, u8 *message)
{
	...
	if(scr_status & MXT_T100_SUPPRESSION) {
...
#ifdef MXT_LPWG
		if (quick_cover_status){	/* drop touch event (out of quick window) */
			if(!((data->ts_data.curr_data[i].x_position > (data->qwindow_size->x_min * 2))
				&& (data->ts_data.curr_data[i].x_position < (data->qwindow_size->x_max * 2))
				&& (data->ts_data.curr_data[i].y_position > (data->qwindow_size->y_min * 2))
				&& (data->ts_data.curr_data[i].y_position < (data->qwindow_size->y_max * 2)))){
